d3 = function() {
  ...
  d3.behavior.zoom = function() {
    ...
    function mousedown() {
      var target = this, event_ = event.of(target, arguments), eventTarget = d3.event.target, moved = 0, w = d3.select(d3_window).on("mousemove.zoom", mousemove).on("mouseup.zoom", mouseup).on("selectstart.zoom", d3_eventCancel), l = location(d3.mouse(target));
      d3_window.focus();
      d3.select("body").style(d3_selection_stylePrefix + "user-select", "none");
      function mousemove() {
        moved = 1;
