d3 = function() {
  ...
  d3.behavior.drag = function() {
    ...
    function mousedown() {
      var target = this, event_ = event.of(target, arguments), eventTarget = d3.event.target, touchId = d3.event.touches ? d3.event.changedTouches[0].identifier : null, offset, origin_ = point(), moved = 0, body, userSelect;
  ...
  d3.behavior.zoom = function() {
    ...
    function mousedown() {
      var target = this, event_ = event.of(target, arguments), eventTarget = d3.event.target, moved = 0, w = d3.select(d3_window).on("mousemove.zoom", mousemove).on("mouseup.zoom", mouseup).on("selectstart.zoom", d3_eventCancel), body, userSelect, l = location(d3.mouse(target));
      d3_window.focus();
      if (body = d3_document.body) {
        userSelect = body.style[d3_vendor + "UserSelect"];
        body.style[d3_vendor + "UserSelect"] = "none";
  ...
  var d3_timer_frame = d3_window.requestAnimationFrame || d3_window[d3_vendor + "RequestAnimationFrame"] || function(callback) {
